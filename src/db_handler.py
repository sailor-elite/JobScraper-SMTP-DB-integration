...
class DataHandler():
    ...
    def insert_data(self, data):
        ...
        sql = '''SELECT * FROM projects WHERE name = ? AND location = ? AND job = ?'''
        cur = self.conn.cursor()
        cur.execute(sql, (name, location, job))
        existing_record = cur.fetchone()

        if existing_record:
            existing_date = existing_record[2]
            if date > existing_date:
                sql = '''UPDATE projects SET date = ? WHERE name = ? AND location = ? AND job = ?'''
                cur.execute(sql, (date, name, location, job))
                self.conn.commit()
            ...
            self.conn.commit()
            print(f"Added new job: '{name}' in location '{location}' with job '{job}'")
